...
def generate_branded_pdf(metadata, info, school_name):
    ...
    p.rect(40, h - 115, w - 80, 25, fill=1, stroke=0.5)
    p.setFillColor(colors.black)
    p.setFont("Helvetica-Bold", 9)
    p.drawString(50, h - 108, f"ID: {info['aid']} | Grade: {info['grade']} | Sub: {info['subject']} | Time: {info['time']} Mins | Difficulty: {info['diff']}/12")
    
    # Questions
    y = h - 150
    for q in metadata.get('questions', []):
        p.setFont("Helvetica-Bold", 11)
        p.drawString(50, y, f"Q{q['id']}. {q['question']}")
        y -= 20
        p.setFont("Helvetica", 10)
...
with tab1:
    if not db.empty:
        ...
        if st.button("🚀 Generate Branded Paper & Diagnostic Key"):
            ...
            with st.spinner("Engineering high-fidelity misconceptions..."):
                ...
                STRICT: NO diagrams. NO repetition of: {avoid}. Text only.
...
with tab2:
    ...
    if uploaded_file and input_aid:
        if input_aid not in st.session_state['meta_store']:
            st.error("ID not found. Ensure you generated it in Phase 1.")
        else:
            ...
            # --- CLASSWISE REPORT ---
            st.subheader("📊 Class-Wise Misconception Summary")
            all_errs = []
            ...
            # --- INDIVIDUAL & REMEDIAL ---
            st.subheader("👤 Individual Report & Detailed Remedial Plan")
            ...
            for q in meta['questions']:
                ...
                with st.container():
                    if s_ans == q['correct']:
                        st.markdown(f"**Q{q['id']}:** ✅ Mastery Confirmed")
                    else:
                        err = q['mappings'].get(s_ans, 'Logic Gap')
                        st.markdown(f"**Q{q['id']}:** ❌ Incorrect (Selected {s_ans})")
                        st.markdown(f"""<div class="remedial-box">
                            <b>Misconception:</b> {err}<br>
                            <b>🛠️ Remedial Plan:</b> Teacher to use physical manipulatives to demonstrate this concept.
